    ...
    public function index()
    {
        ...
        foreach ($clients as $client) {
            $template = new TemplateProcessor(
                storage_path('app/Templates/letter.docx')
            );
            $template->setValue('(Property_Address)', $client->BO_ComAdd);
            $fileName = 'letter_' . $client->id . '.docx';

            $template->saveAs(
                storage_path('app/generated/' . $fileName)
            );
        }
        exit;
